...
const clientUserAgent = `podcastdx client/5.0.0`;
...
class PodcastIndexClient {
  ...
  public async searchPerson(
    ...
    const result = await this.fetch<ApiResponse.SearchPerson>(
      "/search/byperson",
      {
        q: query,
        fulltext: Boolean(options.fulltext),
  ...
  public async recentEpisodes(
    ...
    const result = await this.fetch<ApiResponse.RecentEpisodes>(
      "/recent/episodes",
      {
        ...options,
        max: options.max ?? 10,
  ...
  public async recentFeeds(
    ...
    const result = await this.fetch<ApiResponse.RecentFeeds>(
      "/recent/feeds",
      apiOptions,
    );
    ...
    return {
      ...
      feeds: result.feeds
        ...
        .map((feed) =>
          normalizeKey((lang) => lang.toLowerCase(), "language", feed),
        ),
    };
  }
  ...
  public async recentNewFeeds(
    ...
    const result = await this.fetch<ApiResponse.RecentNewFeeds>(
      "/recent/newfeeds",
      {
        max: options.max ?? 10,
      },
  ...
  public async recentSoundbites(): Promise<ApiResponse.RecentSoundbites> {
    const result = await this.fetch<ApiResponse.RecentSoundbites>(
      "/recent/soundbites",
    );
  ...
  public async podcastByUrl(url: string): Promise<ApiResponse.PodcastByUrl> {
    const result = await this.fetch<ApiResponse.PodcastByUrl>(
      "/podcasts/byfeedurl",
      { url },
    );
  ...
  public async podcastById(id: number): Promise<ApiResponse.PodcastById> {
    const result = await this.fetch<ApiResponse.PodcastById>(
      "/podcasts/byfeedid",
      { id },
    );
    if (!result.feed.categories) {
  ...
  public async podcastByItunesId(
    id: number,
  ): Promise<ApiResponse.PodcastByItunesId> {
    const result = await this.fetch<ApiResponse.PodcastByItunesId>(
      "/podcasts/byitunesid",
      { id },
    );
    if (!result.feed.categories) {
  ...
  public async episodesByFeedUrl(
    ...
    const result = await this.fetch<ApiResponse.EpisodesByFeedUrl>(
      "/episodes/byfeedurl",
      {
        ...rest,
  ...
  public async episodesByFeedId(
    ...
    const result = await this.fetch<ApiResponse.EpisodesByFeedId>(
      "/episodes/byfeedid",
      {
        ...rest,
        since: toEpochTimestamp(since),
  ...
  public async episodesByItunesId(
    ...
    const { since, ...rest } = options;
    const result = await this.fetch<ApiResponse.EpisodesByItunesId>(
      "/episodes/byitunesid",
      {
        ...rest,
        since: toEpochTimestamp(since),
  ...
  public async episodesRandom(
    ...
    const parsedOptions: Record<string, number | string | undefined> =
      options.max ? { max: options.max } : {};

    parsedOptions.lang = Array.isArray(options.lang)
      ? options.lang.join(",")
      : options.lang;
    parsedOptions.cat = Array.isArray(options.cat)
      ? options.cat.join(",")
      : options.cat;
    ...
    const result = await this.fetch<ApiResponse.RandomEpisodes>(
      "/episodes/random",
      parsedOptions,
    );
    return result;
  ...
  public async episodeById(
    ...
    const result = await this.fetch<ApiResponse.EpisodeById>("/episodes/byid", {
      id,
      ...options,
    });
